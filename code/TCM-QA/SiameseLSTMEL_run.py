...
class SiameseNetwork:
    def __init__(self):
        #self.all_path = '/content/drive/MyDrive/TCM-QA/EL data/'
        self.all_path = 'E:\\毕业论文相关\\毕业论文\\代码\\TCM-QA\\EL data\\'
        self.train_path = self.all_path + '相似对语料库train.txt'
        self.vocab_path = self.all_path + 'vocab.txt'
        self.embedding_file = self.all_path + 'token_vec_300.bin'
        self.timestamps_file = self.all_path + 'timestamps.txt'
        self.model_path = self.all_path + 'final326.h5'
    ...
    '''oov的话避免报错，返回查不到'''
    def test_with_check(self,s1,s2):
        flag = 1
        all = list(s1 + s2)
        for i in all:
